...
import { DynamoProjectAdapter } from './adapters/dynamo-project.adapter';
import { DynamoTaskAdapter } from './adapters/dynamo-task.adapater';
import { ProjectController } from './controllers/project.controller';

export class ProjectInfrastructure {
  private static instance: ProjectInfrastructure;
  ...
  private constructor(common: CommonInfrastructure) {
    this.projectRepo = new DynamoProjectAdapter();
    this.taskRepo = new DynamoTaskAdapter();
  ...
  static getInstance(common: CommonInfrastructure): ProjectInfrastructure {
    if (!ProjectInfrastructure.instance) {
      ProjectInfrastructure.instance = new ProjectInfrastructure(common);
    }

    return ProjectInfrastructure.instance;
  }
